...
<?php
	...
	if (!empty(\$_tag)) {
		...
		foreach (\$tlist as \$key => \$value) {
			\$bb = \$tstr ? " OR article.tag LIKE '%".\$value."%'" : "article.tag LIKE '%".\$value."%'";
			\$tstr = \$tstr.\$bb;
		}
		\$whstr = \$whstr." AND (".\$tstr.")";
	}

	if ($flag > 0) {	
		\$whstr = \$whstr.' AND article.flag & $flag = $flag ';	
	}
	\$where['_string'] = \$whstr;
	//分页
	if ($pagesize > 0) {
...
<?php
	...
	if (!empty(\$_tag)) {
		...
		\$tstr = "";
		foreach (\$tlist as \$key => \$value) {
			\$bb = \$tstr ? " OR product.tag LIKE '%".\$value."%'" : "product.tag LIKE '%".\$value."%'";
			\$tstr = \$tstr.\$bb;
		}
		\$whstr = \$whstr." AND (".\$tstr.")";
	}
	...
	if ($flag > 0) {	
		\$whstr = \$whstr.' AND product.flag & $flag = $flag ';	
	}
	\$where['_string'] = \$whstr;
...
<?php
	...
	if (!empty(\$_tag)) {
		...
		\$tstr = "";
		foreach (\$tlist as \$key => \$value) {
			\$bb = \$tstr ? " OR picture.tag LIKE '%".\$value."%'" : "picture.tag LIKE '%".\$value."%'";
			\$tstr = \$tstr.\$bb;
		}
		\$whstr = \$whstr." AND (".\$tstr.")";
	}
	...
	if ($flag > 0) {	
		\$whstr = \$whstr.' AND picture.flag & $flag = $flag ';	
	}
	\$where['_string'] = \$whstr;
	...
	\$piclist['alltitle'] = \$piclist['title'];
...
<?php
	...
	if(empty(\$content['id']) || empty(\$content['cid']) || empty(\$cate['tablename']) ) {
		...
		//上一条记录
        \$_vo=D(ucfirst(\$cate['tablename'].'View'))->where(array(\$cate['tablename'].'.status' => 0, 'id' => array('lt',\$content['id']),'cid' => array('eq',\$content['cid'])))->order('id desc')->find();
...
<?php
	if(empty(\$content['id']) || empty(\$content['cid']) || empty(\$cate['tablename']) ) {
		...
		//下一条记录
        \$_vo=D(ucfirst(\$cate['tablename'].'View'))->where(array(\$cate['tablename'].'.status' => 0,'id' => array('gt',\$content['id']),'cid' => array('eq',\$content['cid'])))->order('id ASC')->find();
