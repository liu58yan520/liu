...
<div class="subnav">
    <div class="content-menu ib-a blue line-x">
        ...
        <a href="" class="onloading on"><em>编辑导航</em></a>
    </div>
    <div class="bk10"></div>
	<div class="form">
		<form method='post' id="form_do" name="form_do" action="<?php echo U(GROUP_NAME. '/Nav/edit');?>">
		<div class="form">
			<dl>
				<dt>导航名称：</dt>
				<dd>
					<input type="text" name="name" class="inp_one" id="name" value="<?php echo ($vo["name"]); ?>"/> 
					&nbsp;&nbsp;站内栏目：
					<select id="selpid">
						<option value="0">自定义导航</option>
						<?php if(is_array($cate)): foreach($cate as $key=>$v): ?><option value="<?php echo ($v["id"]); ?>" <?php if($vo["url"] == '@'.$v['id']): ?>selected<?php endif; ?>><?php echo ($v["delimiter"]); echo ($v["name"]); ?></option><?php endforeach; endif; ?>
					</select>
				</dd>
			</dl>
			<dl>
				<dt>导航链接：</dt>
				<dd>
					<input type="text" name="url" class="inp_large" id="url" value="<?php echo ($vo["url"]); ?>" <?php if($isrelock): ?>readonly='readonly'<?php endif; ?>/>
				</dd>
			</dl>
			<dl>
				<dt>上级导航：</dt>
				<dd>
					<select name="pid">
						...
						<?php if(is_array($nav)): foreach($nav as $key=>$v): ?><option value="<?php echo ($v["id"]); ?>" <?php if($vo["pid"] == $v['id']): ?>selected<?php endif; ?>><?php echo ($v["delimiter"]); echo ($v["name"]); ?></option><?php endforeach; endif; ?>
					</select>
				</dd>
			</dl>
			<dl>
				<dt>导航类型：</dt>
				<dd>
					<input type="radio" value="1" name="navtype" <?php if($vo["navtype"] == '1'): ?>checked<?php endif; ?>>&nbsp;&nbsp;顶部导航&nbsp;&nbsp;
					<input type="radio" value="2" name="navtype" <?php if($vo["navtype"] == '2'): ?>checked<?php endif; ?>>&nbsp;&nbsp;底部导航&nbsp;&nbsp;
					<input type="radio" value="3" name="navtype" <?php if($vo["navtype"] == '3'): ?>checked<?php endif; ?>>&nbsp;&nbsp;其他位置&nbsp;&nbsp;
				</dd>
			</dl>
			<dl>
	            <dt>排序：</dt>
				<dd>
					<input type="text" name="sort" class="inp_one" value="<?php echo ($vo["sort"]); ?>" onkeyup="this.value=this.value.replace(/\D/g,'')"  onafterpaste="this.value=this.value.replace(/\D/g,'')" maxlength="3" size="3"/>
				</dd>
	        </dl>
	        <div class="form_b">	
